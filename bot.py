...
GITHUB_REPO = os.getenv("GITHUB_REPO")  # e.g., "yourname/your-repo"
...
def save_data(data):
    ...
    # Schedule GitHub push (if configured)
    try:
        loop = asyncio.get_running_loop()
        loop.create_task(push_to_github(data))
    except RuntimeError:
        # No running event loop (e.g. called from non‑async context)
        pass
...
async def push_to_github(data: dict):
    """Upload data.json directly to the GitHub repository."""
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            print(f"GitHub fetch SHA failed: {e}")
        ...
        try:
            async with session.put(url, headers=headers, json=payload) as resp:
                if resp.status in (200, 201):
                    print("✅ GitHub data.json updated.")
                else:
...
def create_fund_embed():
    ...
    paid_members = sum(1 for paid in members.values() if paid)
    ...
    if members:
        ...
        for name, paid in members.items():
            member_list.append(f"✅ **{name}** — Paid" if paid else f"❌ **{name}** — Unpaid")
        ...
        for index, chunk in enumerate(chunks):
            field_name = "📋 Member List" if index == 0 else "📋 Member List (Continued)"
...
def create_treasury_embed():
    ...
    if recent:
        lines = [format_transaction_line(entry) for entry in reversed(recent)]
        chunks = []
        current_chunk = ""
        for line in lines:
        ...
        for index, chunk in enumerate(chunks):
            field_name = "🧾 Recent Transactions" if index == 0 else "🧾 Recent Transactions (Continued)"
            embed.add_field(name=field_name, value=chunk, inline=False)
    else:
        embed.add_field(name="🧾 Recent Transactions", value="No transactions yet.", inline=False)

    embed.set_footer(
        text=f"Showing the last {len(recent)} of {len(transactions)} transaction(s) • Use /fund_transcript for the full history"
    )
    return embed
...
async def get_backup_channel():
    ...
    if _backup_channel_override_id:
        try:
            ...
            print(f"BACKUP_CHANNEL_ID is set but not accessible ({e}). Falling back.")
...
async def push_backup():
    global _backup_message_id, _last_backup_ok, _last_backup_error, _last_backup_time
    ...
    try:
        if _backup_message_id:
            try:
                message = await channel.fetch_message(_backup_message_id)
                await message.edit(content="🗄️ Gang fund data backup (auto-updated, do not delete)", attachments=[file])
        ...
        message = await channel.send(content="🗄️ Gang fund data backup (auto-updated, do not delete)", file=file)
            ...
            for attachment in message.attachments:
                ...
                for key in default_data():
                    if key not in restored:
                        restored[key] = default_data()[key]
                data = restored
                save_data(data)
                print(f"Restored gang fund data from Discord backup channel ({len(data['members'])} players).")
                return
    except Exception as e:
...
async def get_transcript_channel():
    ...
    if _transcript_channel_override_id:
        try:
            ...
            print(f"TRANSCRIPT_CHANNEL_ID is set but not accessible ({e}). Falling back.")
    ...
    try:
        ...
        channel = await guild.create_text_channel(
            ...
            overwrites=overwrites,
            reason="Auto-created to keep a permanent transcript of gang treasury activity."
        )
        _transcript_channel = channel
        return _transcript_channel
    except Exception as e:
...
async def post_transcript_entry(entry):
    ...
    embed = discord.Embed(
        title="🟢 Deposit Recorded" if is_deposit else "🔴 Withdrawal Recorded",
...
async def player_autocomplete(interaction: discord.Interaction, current: str):
    ...
    matches = [name for name in data["members"].keys() if current_lower in name.lower()]
...
class FundView(discord.ui.View):
    ...
    async def mark_paid(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_admin(interaction):
            await interaction.response.send_message("❌ You don't have permission to manage the gang fund.", ephemeral=True)
            return
        if not data["members"]:
            await interaction.response.send_message("❌ No players have been added yet.", ephemeral=True)
            return
        options = [discord.SelectOption(label=name[:100], value=name[:100]) for name in data["members"]]
        if len(options) > 25:
            await interaction.response.send_message("❌ There are more than 25 players. Use `/fund_paid` instead.", ephemeral=True)
            return
        view = discord.ui.View(timeout=60)
        view.add_item(PaidSelect(options))
        await interaction.response.send_message("Select the player who paid:", view=view, ephemeral=True)
    ...
    async def mark_unpaid(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        options = [discord.SelectOption(label=name[:100], value=name[:100]) for name in data["members"]]
        if len(options) > 25:
            await interaction.response.send_message("❌ There are more than 25 players. Use `/fund_unpaid` instead.", ephemeral=True)
            return
        view = discord.ui.View(timeout=60)
        view.add_item(UnpaidSelect(options))
        await interaction.response.send_message("Select the player who is unpaid:", view=view, ephemeral=True)
...
class DepositModal(discord.ui.Modal, title="Deposit to Gang Fund"):
    ...
    reason_input = discord.ui.TextInput(label="Reason", placeholder="e.g. Weekly member dues...", style=discord.TextStyle.paragraph, required=False, max_length=300)

    async def on_submit(self, interaction: discord.Interaction):
        amount = parse_amount(self.amount_input.value)
        if amount is None:
            await interaction.response.send_message("❌ Enter a valid whole number greater than 0.", ephemeral=True)
            return
...
class WithdrawModal(discord.ui.Modal, title="Withdraw from Gang Fund"):
    ...
    reason_input = discord.ui.TextInput(label="Reason", placeholder="e.g. Bought supplies...", style=discord.TextStyle.paragraph, required=False, max_length=300)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if amount > data.get("treasury_balance", 0):
            await interaction.response.send_message(f"❌ Insufficient funds. Current balance: **${data.get('treasury_balance', 0):,}**.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
...
class TreasuryView(discord.ui.View):
    ...
    async def deposit(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not is_admin(interaction):
            await interaction.response.send_message("❌ You don't have permission to manage the gang treasury.", ephemeral=True)
            return
...
async def fund_add(interaction: discord.Interaction, player: discord.Member):
    ...
    if name in data["members"]:
        await interaction.followup.send(f"❌ **{name}** is already in the fund.", ephemeral=True)
        return

    data["members"][name] = False
    save_data(data)
    await update_all_panels()
    await interaction.followup.send(f"✅ Added **{name}** to the gang fund.", ephemeral=True)
...
async def fund_remove(interaction: discord.Interaction, player: str):
    ...
    if player not in data["members"]:
        await interaction.followup.send(f"❌ **{player}** is not in the fund.", ephemeral=True)
        return
    del data["members"][player]
    save_data(data)
    await update_all_panels()
    await interaction.followup.send(f"🗑️ Removed **{player}** from the gang fund.", ephemeral=True)
...
async def fund_paid(interaction: discord.Interaction, player: str):
    ...
    save_data(data)
    await update_all_panels()
    await interaction.followup.send(f"✅ **{player}** has paid the gang fund.", ephemeral=True)
...
async def fund_unpaid(interaction: discord.Interaction, player: str):
    ...
    save_data(data)
    await update_all_panels()
    await interaction.followup.send(f"❌ **{player}** is now marked as unpaid.", ephemeral=True)
...
@bot.tree.command(name="fund_message", description="Set or clear a custom note shown at the bottom of the fund embed")
@app_commands.describe(text="The note to show (leave empty to clear)")
async def fund_message(interaction: discord.Interaction, text: str = ""):
    ...
    await update_all_panels()
    if data["message"]:
        await interaction.followup.send(f"📝 Fund message updated to:\n> {data['message']}", ephemeral=True)
    else:
        await interaction.followup.send("🧹 Fund message cleared.", ephemeral=True)
...
@bot.tree.command(name="fund_reset", description="Mark all players as unpaid again")
...
class ConfirmResetListView(discord.ui.View):
    ...
    @discord.ui.button(label="Yes, clear the list", style=discord.ButtonStyle.danger, emoji="🗑️")
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.confirmed = True
        self.stop()
        await interaction.response.edit_message(content="🗑️ Clearing the player list...", view=None)

    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.secondary)
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.confirmed = False
        self.stop()
        await interaction.response.edit_message(content="❌ Cancelled. Player list unchanged.", view=None)


@bot.tree.command(name="fund_reset_list", description="Permanently remove ALL players from the gang fund list")
async def fund_reset_list(interaction: discord.Interaction):
    ...
    if not is_admin(interaction):
        ...
        return
    if not data["members"]:
        await interaction.followup.send("❌ The player list is already empty.", ephemeral=True)
        return
    view = ConfirmResetListView(owner_id=interaction.user.id)
    await interaction.followup.send(
        f"⚠️ This will permanently remove all **{len(data['members'])}** players from the list. This cannot be undone. Are you sure?",
...
@bot.tree.command(name="fund_backup_status", description="Check whether gang fund data backups are working")
async def fund_backup_status(interaction: discord.Interaction):
    ...
    if channel:
        ...
        embed.add_field(name="Backup Channel", value="❌ Not available — bot likely needs Manage Channels permission.", inline=False)

    if _last_backup_time is not None:
        status_text = "✅ Success" if _last_backup_ok else f"❌ Failed — {_last_backup_error}"
        embed.add_field(name="Last Backup Attempt", value=f"{status_text}\n<t:{int(_last_backup_time.timestamp())}:R>", inline=False)
    else:
        embed.add_field(name="Last Backup Attempt", value="No backup attempted yet this session.", inline=False)

    embed.add_field(name="Local data.json", value="Exists" if os.path.exists(DATA_FILE) else "Missing", inline=True)
    embed.add_field(name="Players Tracked", value=str(len(data["members"])), inline=True)
    await interaction.followup.send(embed=embed, ephemeral=True)


@bot.tree.command(name="fund_backup_now", description="Force an immediate backup of the gang fund data")
async def fund_backup_now(interaction: discord.Interaction):
    ...
    await push_backup()
    if _last_backup_ok:
        await interaction.followup.send(f"✅ Backup saved successfully to {_backup_channel.mention if _backup_channel else 'the backup channel'}.", ephemeral=True)
    else:
...
@bot.tree.command(name="fund_treasury", description="Create the gang treasury panel")
async def fund_treasury(interaction: discord.Interaction):
    ...
    await interaction.followup.send("✅ Gang treasury panel created!", ephemeral=True)


@bot.tree.command(name="fund_deposit", description="Deposit money into the gang treasury")
@app_commands.describe(amount="Amount to deposit", reason="Why this money is being deposited")
...
@app_commands.describe(amount="Amount to withdraw", reason="Why this money is being withdrawn")
async def fund_withdraw(interaction: discord.Interaction, amount: int, reason: str = ""):
    ...
    if amount <= 0:
        ...
        return
    if amount > data.get("treasury_balance", 0):
        await interaction.followup.send(f"❌ Insufficient funds. Current balance: **${data.get('treasury_balance', 0):,}**.", ephemeral=True)
        return
...
def create_transcript_page_embed(page: int):
    ...
    embed = discord.Embed(
        ...
        description=f"Current balance: **${data.get('treasury_balance', 0):,}** • {total} total transaction(s)",
    ...
    if page_items:
        ...
        embed.add_field(name="No transactions yet", value="Use the Deposit / Withdraw buttons on the treasury panel.", inline=False)
...
class TranscriptPaginatorView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message("❌ This isn't your transcript view.", ephemeral=True)
            return False
        return True
    ...
    async def previous(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.page -= 1
        embed, self.page, _ = create_transcript_page_embed(self.page)
...
async def fund_transcript(interaction: discord.Interaction):
    ...
    view = TranscriptPaginatorView(owner_id=interaction.user.id)
    view.page = page
    if total_pages <= 1:
...
class AuthorModal(discord.ui.Modal, title="Author"):
    def __init__(self, builder_view):
        ...
        self.name_input = discord.ui.TextInput(label="Author name", required=False, max_length=256, default=current_author.name if current_author else "")
        self.icon_input = discord.ui.TextInput(label="Author icon URL", required=False, default=current_author.icon_url if current_author else "")
        self.url_input = discord.ui.TextInput(label="Author link URL", required=False, default=current_author.url if current_author else "")
...
class FooterModal(discord.ui.Modal, title="Footer"):
    def __init__(self, builder_view):
        ...
        self.builder_view = builder_view
        embed = builder_view.current_embed()
        self.text_input = discord.ui.TextInput(label="Footer text", required=False, max_length=2048, default=embed.footer.text if embed.footer else "")
        self.icon_input = discord.ui.TextInput(label="Footer icon URL", required=False, default=embed.footer.icon_url if embed.footer else "")
...
class ImagesModal(discord.ui.Modal, title="Images"):
    def __init__(self, builder_view):
        ...
        self.builder_view = builder_view
        embed = builder_view.current_embed()
        self.image_input = discord.ui.TextInput(label="Large image URL", required=False, default=embed.image.url if embed.image else "")
        self.thumbnail_input = discord.ui.TextInput(label="Thumbnail URL (small, top-right)", required=False, default=embed.thumbnail.url if embed.thumbnail else "")
...
class EmbedSwitchSelect(discord.ui.Select):
    def __init__(self, builder_view):
        self.builder_view = builder_view
        options = [
            discord.SelectOption(label=f"Embed {i + 1}", value=str(i), default=(i == builder_view.active_index))
